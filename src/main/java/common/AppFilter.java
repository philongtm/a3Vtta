...
public class AppFilter implements Filter {
	...
	private boolean sessionCheck(HttpServletRequest request, HttpServletResponse response, HttpSession session) 
		...
		if( (method.equals(GET)==false) && (method.equals(POST)==false) ) {
		...
		if( path.equals(LOGIN_JSP) || path.equals(LOGIN_ACTION) ) {
			...
			if(session==null){
				...
				if( ValidSession1(session)==false ) {
		...
		//セッションＩＤ：なし
		if( (request.isRequestedSessionIdFromCookie()==false) && (request.isRequestedSessionIdFromURL()==false) ) {
			//無効セッション：４０４エラー
