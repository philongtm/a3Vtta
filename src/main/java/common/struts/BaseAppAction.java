 ...
 *
 */
public abstract class BaseAppAction<F extends ActionForm> {
     ...
     */
    protected BaseAppAction(Class<F> formClass, String formBeanName) {
        this.formClass = formClass;
        this.formBeanName = formBeanName;
    }

    private void saveActionFormToSession(HttpServletRequest request, F actionForm) throws Exception {
        if (StringUtils.hasText(formBeanName)) {
            request.getSession().setAttribute(formBeanName, actionForm);
        }
    }
    ...
    public final String execute(@ModelAttribute F form, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
        ...
        if (findForward instanceof TempFile) {
            ...
            return GS.A00_DOWNLOAD + GS.ACTION;
        }
        //その他
        return null;
    }
