...
/**
 *  OZ6109_査定進捗照会タブ DBアクセスクラス <br>
 */
public class SateiSincyokuDbAcc extends CommonDbAcc {

    private TorihikisakiBean tori_bean = null;      			// 取引先情報
    private SateiSincyokuForm form = null;                 	// アクションフォーム
    //IT159対応
	...
	private static final String LINK_FLG					= "link_flg";		// 汎用1
    private static final String SOSHIKI					= "soshiki";		// 組織キー
    private static final String ORA						= "ora";			// 組織値
    ...
    private static final String OPE_KBN 					= "ope_kbn";		// 入力区分
    ...
    private static final String SP_SS_OZ6109_SELECT_ANKEN_NO = "SP_SS_OZ6109_SELECT_ANKEN_NO";	// 取引先の査定案件を取得する
    private static final String SP_SS_OZ6109_SELECT_T1300    = "SP_SS_OZ6109_SELECT_T1300"; 		// 取引先の進捗一覧を取得する
     ...
     */
    public SateiSincyokuDbAcc(SqlExecuter sqlExec, Log log, AppContext appcontext) {
        super(sqlExec, log);

        //ビーン取得
        SessionData cmnData = appcontext.getCMN();
        tori_bean = cmnData.getTori_bean();
        form = (SateiSincyokuForm)appcontext.getActionForm();
    ...
    public void getAnkenNo() throws SQLException {
    	...
    	ResultSet rs = null;
        cstmt = new ExCallableStatement(SP_SS_OZ6109_SELECT_ANKEN_NO, sqlExec);
     ...
     * @exception SQLException
     */
    public void getHandanList() throws SQLException {
        
        //ExCallableStatement生成
    	ExCallableStatement cstmt = null;
    	ResultSet rs = null;
        cstmt = new ExCallableStatement(SP_SS_OZ6109_SELECT_T1300, sqlExec);
        cstmt.setStringIn(comLangMode);
        cstmt.setStringIn(systemKbn);
        cstmt.setStringIn(sateiKaishaCd);
        cstmt.setStringIn(init_bunrui2);
        cstmt.setStringIn(form.getAnken_no_ser());
        cstmt.setResultSet(RESULTSET);
