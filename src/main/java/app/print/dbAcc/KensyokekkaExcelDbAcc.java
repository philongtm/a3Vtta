...
003		2009/12/18		SSC				課題No.211 金額出力箇所をlongで出力するよう、修正
004		2011/07/05		SSC				案件No.D9059 補正後引当控除後残高削除(空白)対応
005		2016/04/06		SSC				部門廃止対応(二次) 引当金額集計表(検証結果)に「本部」欄を追加
******************************************************************************/
...
*/
public class KensyokekkaExcelDbAcc extends CommonDbAcc {
	...
	//引当金検証データ計算式
	private static final String FORMULA1 = "SUM(L{1}:W{1})";
	private static final String FORMULA2 = "X{1}+Y{1}";
	private static final String FORMULA3 = "IF(Z{1}<SUM(AA{1}:AB{1}),IF(Z{1}<0,0,Z{1}),SUM(AA{1}:AB{1}))"; 
	private static final String FORMULA4 = "SUM(AC{1}:AE{1})";
	private static final String FORMULA5 = "Z{1}-(AC{1}+AD{1}+AE{1})+AH{1}-AI{1}";
	...
	//査定データ計算式
	private static final String FORMULA8 = "SUM(BF{1}:BQ{1})";
	private static final String FORMULA9 = "BR{1}+BS{1}";//
	private static final String FORMULA10 = "IF(BT{1}<SUM(BU{1}:BV{1}),IF(BT{1}<0,0,BT{1}),SUM(BU{1}:BV{1}))";
	private static final String FORMULA11 = "SUM(BW{1}:BY{1})";
	private static final String FORMULA12 = "BT{1}-(BW{1}+BX{1}+BY{1})+CB{1}-CC{1}";
	private static final String FORMULA13 = "CC{1}+CE{1}";

	//プロシージャ
	private static final String SP_SS_L_SELECT_CHOHYOHEAD2 	= "SP_SS_L_SELECT_CHOHYOHEAD2";	 //帳票ヘッダ部取得プロシージャ
	private static final String SP_SS_LD2102_SELECT_ICHIRAN4 	= "SP_SS_LD2102_SELECT_ICHIRAN4"; //一覧情報取得プロシージャ
	...
	public KensyokekkaExcelDbAcc(SqlExecuter sqlExec, Log log, AppContext appcontext) {
		super(sqlExec, log);
	...
	public void getHeader(Excel excel) throws SQLException {
		...
		try{
			...
			if(rs.next()){
				//作成日
				excel.selectCell(1,85,true);
				excel.setCellValue(rs.getString(SAKUSEI_DT));
				
				// 課題No.193
				// 追加開始
				//分類１
				excel.selectCell(2,85,true);
	...
	public boolean getMeisai(Excel excel) throws SQLException {
		...
		ExCallableStatement exCstmt = new ExCallableStatement(SP_SS_LD2102_SELECT_ICHIRAN4, sqlExec);
				...
				for (int j=startClm; j<=endClm; j++ ) {
    				...
    				switch (j){
	    				...
	    				case 25:
			    			...
			    			excel.setCellFormula(formula);
			    			break;
	    				case 26: excel.setCellValue(Function.getValueOfLong(rs.getString(RYUHOSAIMU)));break;
	    				case 27: excel.setCellValue(Function.getValueOfLong(rs.getString(OTH_RYUHOSAIMU)));break;
	    				case 28: 
			    			formula = Function.replaceExpression(FORMULA3,String.valueOf(i+1));
			    			excel.setCellFormula(formula);
			    			break;
	    				case 29: excel.setCellValue(Function.getValueOfLong(rs.getString(HOZEN)));break;
	    				case 30: excel.setCellValue(Function.getValueOfLong(rs.getString(SONOTAKAISYU)));break;
	    				case 31: 
			    			formula = Function.replaceExpression(FORMULA4,String.valueOf(i+1));
			    			excel.setCellFormula(formula);
			    			break;	    				
	    				case 32: excel.setCellValue(Function.getValueOfLong(rs.getString(HOSYO_SAIMUKEI)));break;
	    				case 33: excel.setCellValue(Function.getValueOfLong(rs.getString(RIKO_KENEN)));break;
	    				...
	    				case 44: excel.setCellValue(rs.getString(KTK_1));break;
	    				case 45: excel.setCellValue(rs.getString(OYA_KTK_1));break;
	    				case 46: excel.setCellValue(rs.getString(OYA_BUSINESS_NM_1));break;
	    				case 47: excel.setCellValue(rs.getString(TAIRYU_KBN_1));break;
	    				case 48: excel.setCellValue(rs.getString(KESSANKI_1));break;
	    				case 49: excel.setCellValue(rs.getString(TEI_KTK));break;
	    				case 50: excel.setCellValue(rs.getString(YOTYUUI));break;
	    				case 51: excel.setCellValue(rs.getString(AKAJI));break;
	    				case 52: excel.setCellValue(rs.getString(SAIMU_TYOKA));break;
	    				case 53: excel.setCellValue(rs.getString(M12));break;
	    				case 54: excel.setCellValue(rs.getString(KASIDAORE_KENEN));break;
	    				case 55: excel.setCellValue(rs.getString(HASAN_KOSEI));break;
	    				case 56: excel.setCellValue(rs.getString(RIKI_SITEI));break;
	    				case 57: excel.setCellValue(Function.getValueOfLong(rs.getString(UKETORI_TEGATA_1)));break;
	    				case 58: excel.setCellValue(Function.getValueOfLong(rs.getString(YUSYUTU_UKETORI_TEGATA_1)));break;
	    				case 59: excel.setCellValue(Function.getValueOfLong(rs.getString(URIKAKEKIN_1)));break;
	    				case 60: excel.setCellValue(Function.getValueOfLong(rs.getString(TORIHIKI_MAEWATASHIKIN_1)));break;
	    				case 61: excel.setCellValue(Function.getValueOfLong(rs.getString(TATEKAEKIN_1)));break;
	    				case 62: excel.setCellValue(Function.getValueOfLong(rs.getString(MISYUUNYUUKIN_1)));break;
	    				case 63: excel.setCellValue(Function.getValueOfLong(rs.getString(MISYUUSYUEKI_1)));break;
	    				case 64: excel.setCellValue(Function.getValueOfLong(rs.getString(TANKI_KASITUKEKIN_1)));break;
	    				case 65: excel.setCellValue(Function.getValueOfLong(rs.getString(SASHIIRE_HOSYOKIN_1)));break;
	    				case 66: excel.setCellValue(Function.getValueOfLong(rs.getString(KARIBARAIKIN_1)));break;
	    				case 67: excel.setCellValue(Function.getValueOfLong(rs.getString(TYOKI_KASITUKEKIN_1)));break;
	    				case 68: excel.setCellValue(Function.getValueOfLong(rs.getString(SONOTA_TOUSI_1)));break;
	    				case 69:
			    			formula = Function.replaceExpression(FORMULA8,String.valueOf(i+1));
			    			excel.setCellFormula(formula);
			    			break;
	    				case 70: excel.setCellValue(Function.getValueOfLong(rs.getString(KOTEIKA_EIGYOU_SAIKEN_1)));break;
	    				case 71:
			    			formula = Function.replaceExpression(FORMULA9,String.valueOf(i+1));
			    			excel.setCellFormula(formula);
			    			break;
	    				case 72:excel.setCellValue(Function.getValueOfLong(rs.getString(RYUHOSAIMU_1)));break;
	    				case 73:excel.setCellValue(Function.getValueOfLong(rs.getString(OTH_RYUHOSAIMU_1)));break;
	    				case 74: 
			    			formula = Function.replaceExpression(FORMULA10,String.valueOf(i+1));
			    			excel.setCellFormula(formula);
			    			break;
	    				case 75:excel.setCellValue(Function.getValueOfLong(rs.getString(HOZEN_1)));break;
	    				case 76:excel.setCellValue(Function.getValueOfLong(rs.getString(SONOTAKAISYU_1)));break;
	    				case 77: 
			    			formula = Function.replaceExpression(FORMULA11,String.valueOf(i+1));
			    			excel.setCellFormula(formula);
			    			break;	    				
	    				case 78:excel.setCellValue(Function.getValueOfLong(rs.getString(HOSYO_SAIMUKEI_1)));break;
	    				case 79:excel.setCellValue(Function.getValueOfLong(rs.getString(RIKO_KENEN_1)));break;
	    				case 80:excel.setCellValue(Function.getValueOfLong(rs.getString(KIHIKIATEKIN_1)));break;
	    				case 81:
	    				...
	    				case 82:excel.setCellValue(Function.getValueOfLong(rs.getString(TUIKA_HIKIATE_1)));break;
	    					// 追加完了
	    				case 83:
	    					formula = Function.replaceExpression(FORMULA13,String.valueOf(i+1));
	    					excel.setCellFormula(formula);
	    					break;
	    				case 84:excel.setCellValue(rs.getString(HIKIATEKIN_HANTEIKONKYO_1));break;
	    				case 85:excel.setCellValue(rs.getString(SYORI_DT_1));
	    				break;
