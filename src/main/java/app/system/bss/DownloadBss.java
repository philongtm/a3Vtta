/******************************************************************************
 ...
 006		2025/09/12		STi				F4.8.0_プラネット債権査定基準変更対応
 ******************************************************************************/
...
//	private static final String KENSYOKEKKA_EN      = "016";	//Reserve Amount Total Table(Verification)(For PN)
    ...
    //債権査定基準変更対応
    //追加開始
    private static final String HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI			= "0131";		//引当金額集計表(査定結果)(国内用：初回月) 0131
    private static final String HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI		= "0132";		//引当金額集計表(査定結果)(国内用：初回月以外) 0132
    private static final String HIKIATEKIN_KENSHOKEKKA_KOKUNAI					= "0133";		//引当金額集計表(検証結果)(国内用) 0133
    //追加完了
    //2025/9/12 F4.8.0_ここまで
    ...
    //債権査定基準変更対応
    //追加開始
    private static final String LIST_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI			= "10";	//引当金額集計表(査定結果)(国内用：初回月)
    private static final String LIST_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI		= "11";	//引当金額集計表(査定結果)(国内用：初回月以外)
    private static final String LIST_HIKIATEKIN_KENSHOKEKKA_KOKUNAI					= "12";	//引当金額集計表(検証結果)(国内用)
    //2025/9/12 F4.8.0_ここまで
    ...
    //債権査定基準変更対応
    //追加開始
    private static final String CHOHYO_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI			= "0131";		//引当金額集計表(査定結果)(国内用：初回月) 0131
    private static final String CHOHYO_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI		= "0132";		//引当金額集計表(査定結果)(国内用：初回月以外) 0132
    private static final String CHOHYO_HIKIATEKIN_KENSHOKEKKA_KOKUNAI				= "0133";		//引当金額集計表(検証結果)(国内用) 0133
    //追加完了
    //2025/9/12 F4.8.0_ここまで
...
//			//追加完了
        ...
        //「引当金額集計表（検証結果）（国内用）」を選択した場合、〆区分セレクトボックスを表示
        if(HIKIATEKIN_KENSHOKEKKA_KOKUNAI.equals(list_type)) {
            //2025/09/12_F4.8.0_ここまで
    ...
    public String download() throws Exception {
        ...
        if(LIST_TAIRYUMEISAI.equals(selected_list)){
            ...
            //2025/09/12_F4.8.0_名称直接修正
            //債権査定基準変更対応
            //追加開始

        }else if(CHOHYO_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI.equals(form.getList_type())){
            //引当金額集計表(査定結果)(国内用：初回月)
            app.print.bss.SateikekkaHonsyaSyokaiExcelBss bss = new app.print.bss.SateikekkaHonsyaSyokaiExcelBss(appContext);
            bss.execute();
        }else if(CHOHYO_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI.equals(form.getList_type())){
            //引当金額集計表(査定結果)(国内用：初回月以外)
            app.print.bss.SateikekkaHonsyaSyokaiIgaiExcelBss bss = new app.print.bss.SateikekkaHonsyaSyokaiIgaiExcelBss(appContext);
            bss.execute();
        }else if(CHOHYO_HIKIATEKIN_KENSHOKEKKA_KOKUNAI.equals(form.getList_type())){
            //引当金額集計表(検証結果)(国内用)
...
//				//追加完了
        }else if(HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI.equals(list_type)					//引当金額集計表(査定結果)(国内用：初回月)
                || HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI.equals(list_type)){		//引当金額集計表(査定結果)(国内用：初回月以外追加)
            //2025/09/12_F4.8.0_ここまで
...
//				//追加完了
        }else if(HIKIATEKIN_KENSHOKEKKA_KOKUNAI.equals(list_type)){						//引当金額集計表(検証結果)(国内用)
            //2025/09/12_F4.8.0_ここまで
        ...
        //2025/09/12_F4.8.0
        //出力言語タイプ設定
        if(TAIRYUMEISAI_HO.equals(list_type)
//    			|| SATEIKEKKA_HO.equals(list_type) || KENSYOKEKKA.equals(list_type)
                ...
                //債権査定基準変更対応
                //追加開始
                || HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI.equals(list_type)		//引当金額集計表(査定結果)(国内用：初回月)
                || HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI.equals(list_type)	//引当金額集計表(査定結果)(国内用：初回月以外追加)
                || HIKIATEKIN_KENSHOKEKKA_KOKUNAI.equals(list_type)){			//引当金額集計表(検証結果)(国内用)
            //追加完了
            //2025/09/12_F4.8.0_ここまで
    ...
    public void setStyle() throws Exception {
        ...
        if(LIST_TAIRYUMEISAI_HO.equals(selectedList) || LIST_TAIRYUMEISAI.equals(selectedList)
                ...
                //債権査定基準変更対応
                //追加開始
                || LIST_HIKIATEKIN_KENSHOKEKKA_KOKUNAI.equals(selectedList))
        {
            //追加完了
            //2025/09/12_F4.8.0_ここまで
...
//    			|| LIST_BS_TAIHI.equals(selectedList) || LIST_BS_SYOKAI.equals(selectedList)
                ...
                //債権査定基準変更対応
                //追加開始
                || LIST_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI.equals(selectedList)
                || LIST_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI.equals(selectedList)){
            //追加完了
            //2025/09/12_F4.8.0_ここまで
    ...
    public boolean isRelateChk(String selected_list) throws Exception {
        ...
        if(LIST_TAIRYUMEISAI_HO.equals(selected_list) || LIST_TAIRYUMEISAI.equals(selected_list)
                ...
                //債権査定基準変更対応
                //追加開始
                || LIST_HIKIATEKIN_KENSHOKEKKA_KOKUNAI.equals(selected_list)
            //追加完了
            //2025/09/12_F4.8.0_ここまで
        ){
...
//    			|| LIST_BS_TAIHI.equals(selected_list) || LIST_BS_SYOKAI.equals(selected_list)
                ...
                //債権査定基準変更対応
                //追加開始
                || LIST_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAI.equals(selected_list)
                || LIST_HIKIATEKIN_SATEIKEKKA_KOKUNAI_SYOKAIIGAI.equals(selected_list)
            //追加完了
            //2025/09/12_F4.8.0_ここまで
        ){
