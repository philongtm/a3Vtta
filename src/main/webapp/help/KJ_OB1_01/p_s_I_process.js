...
this.MouseDouble=MouseDouble;
            ...
            function MouseDouble(nsEvent)
            {
	            ...
	            if((i=getEqualAction(items, nsEvent))!=-1)
	            {
		            ...
		            if(items[i].type=="M"&&items[i].actionParam.indexOf("Double")!=-1)
		            {
			            ...
			            if (isIE)
			            {//IE
			                if (compareDoubleMouseCommandIE(items[i].command, contentFrame))
			                {
			                    isCorrect = true;
			                    goToStep(items[i].nextUrl);
			                }
			            ...
			            {//Netscape
			                if (compareDoubleMouseCommandNS(items[i].command, nsEvent))
			                {
			                    isCorrect = true;
			                    goToStep(items[i].nextUrl);
			                }
           ...
           function compareDoubleMouseCommandIE(strCommand, win)
            {
            var strKeyOne = "";
            var strKeyTwo = "";
            var strTemp1 = "";
            if (win.event.ctrlKey) strKeyOne += "[17]";
            if (win.event.altKey) strKeyOne += "[18]";
            if (win.event.shiftKey) strKeyOne += "[16]";
            if (win.event.ctrlKey) strKeyTwo += "[17]";
            if (win.event.altKey) strKeyTwo += "[18]";
            if (win.event.shiftKey) strKeyTwo += "[16]";
            strTemp1.replace([16], "[Shift]");
            strTemp1.replace([17], "[Control]");
            strTemp1.replace([18], "[Alt]");
            strKeyOne += "[1]";
            strKeyTwo += "[2]";
            if (strKeyOne == strCommand) 
            {
            strTemp1 += "[left Click]"; 
            ...
            function compareDoubleMouseCommandNS(strCommand, event)
            {
               var strKey = "";
               switch(event.modifiers) {
                case 1:	strKeyOne += "[18]"; strKeyTwo += "[18]"; break;
                case 2:	strKeyOne += "[17]"; strKeyTwo += "[17]"; break;
                case 3:	strKeyOne += "[17][18]"; strKeyTwo += "[17][18]"; break;
                case 4:	strKeyOne += "[16]"; strKeyTwo += "[16]"; break;
                case 5:	strKeyOne += "[18][16]"; strKeyTwo += "[18][16]"; break;
                case 6:	strKeyOne += "[17][16]"; strKeyTwo += "[17][16]"; break;
                case 7:	strKeyOne += "[17][18][16]"; strKeyTwo += "[17][18][16]"; break;
            }
            strKeyOne += "[1]";
            strKeyTwo += "[2]";
            if (strKeyOne == strCommand) 
