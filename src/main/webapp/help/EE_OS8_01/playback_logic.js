...
function EmailReport1Admin()
{
   ...
   TestInfo.strSubject = strLessonName + "-" + 'Report';
...
catch(e)
{
 ...
 TestInfo.Percentage = Percentage + 'Point';
 TestInfo.Passed = Passed ;
   strReport = 'BEGIN REPORT' + "%0D%0A";
   strReport += 'Date' + ": " + TestInfo.strDate + "%0D%0A";
   strReport += 'Lesson Name' + ": " + strMailLessonName + "%0D%0A";
   strReport += 'Author' + ": " + strMailAuthor + "%0D%0A";
   strReport += 'Execution Time' + ": " + TestInfo.TotalTime + "%0D%0A";
   strReport += 'Number of Questions' + ": " + parseInt(TestInfo.CorrectSteps+TestInfo.IncorrectSteps) + "%0D%0A";
   strReport += 'Number of Correct Answers' + ": " + TestInfo.CorrectSteps + "%0D%0A";
   strReport += 'Number of Incorrect Answers' + ": " + TestInfo.IncorrectSteps + "%0D%0A";
   if (GetFalseStepCount()>0)
       strReport += 'Inaccurate Step List' + ": " + GetFalseSteps() + "%0D%0A";
   strReport += 'Score' + ": " + TestInfo.Percentage.replace(/%/g,"persent") + "%0D%0A";
   if (nRequiredPercent>0)
       strReport += 'Acceptance Line' + ": " + TestInfo.RequiredPercentage + "%0D%0A";
   strReport += "%0D%0A" + 'Details' + ":%0D%0A";

   var strDetailsHTML="";
   strDetailsHTML += 'Step ID' + "%09" + 'Number of Trials' + "%09" + 'Judgement';
   for (var i=0; i < TestInfo.Details.length; i++)
   {
       ...
       ((TestInfo.Details[i][5]==true) ?(TestInfo.Details[i][2]==true?'Correct':'Incorrect'): '-');
   }
   ...
   if (strRet=="Continue")
   {
       ...
       repWin.close();
       var strMailTo = "mailto:" + strMailReportEmail + "?subject=" + strMailLessonName + "-" + 'Report' + "&body=" + TestInfo.strContent;
...
catch(e)
{
 ...
 TestInfo.Passed = Passed;
   ...
   {
       var langPercentMark = "Point";
       var langReport = "Report";
...
function updateSCOUser_GradeInfo(nPercent)
{
   try
   {
       ...
       var s_lesson_status=doLMSGetValue("cmi.core.lesson_status");
...
function commitData()
{
   ...
   strLMSStatus = "incomplete";
   if((intCompletionState == 0) && (isAutoDemonstration()))
   {
       strLMSStatus = "completed";
   }
   if((intCompletionState == 1) && !(isAutoDemonstration()||isOperationExercise()||isPractice() || isOperationLecture()))
   {
       strLMSStatus = "completed";
   }
   ...
   if ((intCompletionState == 4) && (isOperationLecture()))
   {
       strLMSStatus = "completed";
   }
   doLMSSetValue("cmi.core.lesson_status", strLMSStatus);
   doLMSCommit();
   doLMSFinish();
}
