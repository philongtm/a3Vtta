...
<bean:define id="KureemuForm" name="04KureemuForm" type="app.system.form.KureemuForm" />
<% Pager pager = KureemuForm.getPager(); %>
</HEAD>
<BODY>
<CENTER>
<%--ヘッダ部分--%>
<DIV id="main">
	...
	<%--コンテンツ部分--%>
	<DIV id="contents">
		<H1 class="title01"><%=i18n.get(GL.TITLE_OS3101)%></H1>
		...
		<DIV id="list">
			<html:form action="/system/kureemu" >
				...
				<DIV class="rightbox">
					...
					<TABLE class="titl">
						...
						<TR><TD class="nonBorder"><%=i18n.get(GL.OS3101_SAISETTEIKENSUU)%></TD></TR>
					</TABLE>
					<%} else {%>
					<TABLE class="titl">
						<TR><TD class="nonBorder">&nbsp;</TD></TR>
						<TR><TD class="nonBorder"><DIV class="top10"><%=i18n.get(GL.OS3101_SAISETTEIKENSUU)%></DIV></TD></TR>
					</TABLE>
					<%}%>
					<TABLE class="tbl">
						<TR>
						...
						<TR>
							<TD><bean:write name="KureemuForm" property="kuremusaiken_misyori" /></TD>
							<TD><bean:write name="KureemuForm" property="kuremusaiken_syorityu" /></TD>
							<TD><bean:write name="KureemuForm" property="kuremusaiken_syoninmati" /></TD>
							<TD><bean:write name="KureemuForm" property="kuremusaiken_kanryo" /></TD>
						</TR>
					</TABLE>
				</DIV>
						...
						<TR style="width:100%;">
							...
							<TD style="width:14%;">
								<html:select property="sateiki" onchange="doSubmit('sateiki')" style="width:80">
									<html:optionsCollection name="KureemuForm" property="ar_sateiki" value="value" label="key" /></html:select>
							</TD>
							...
							<TD style="width:13%;">
								<html:select property="sort_item" onchange="doSubmit('sort_item')" style="width:100">
									<html:optionsCollection name="KureemuForm" property="ar_sort_item" value="value" label="key" /></html:select>
							</TD>
							<%-- 整列方向 --%>
							<TD style="width:3%;">
								<html:select property="sort_order" onchange="doSubmit('sort_order')" style="width:70">
									<html:optionsCollection name="KureemuForm" property="ar_sort_order" value="value" label="key" /></html:select>
							</TD>
							...
							<TD style="width:9%;"><html:select property="view" onchange="doSubmit('show')" style="width:70">
								<html:optionsCollection name="KureemuForm" property="ar_show" value="value" label="key" /></html:select>
							</TD>

							<%-- ←前のXX件 --%>
							<TD style="width:13%;">
								<logic:notEqual name="KureemuForm" property="x" value="">
									<%if(SESSION_DATA_APP.getComLangMode().equals("Ja")) {%>
										<a href="#" onClick="doSubmit('prevX')"><bean:write name="KureemuForm" property="x" /></a>
									<%} else {%>
										<a href="#" onClick="doSubmit('prevX')"><bean:write name="KureemuForm" property="xen" /></a>
									<%}%>
								</logic:notEqual>
							</TD>
							...
							<%-- 次のXX件→ --%>
							<TD style="width:14%;">
								<logic:notEqual name="KureemuForm" property="y" value="">
									<%if(SESSION_DATA_APP.getComLangMode().equals("Ja")) {%>
										<a href="#" onClick="doSubmit('nextY')"><bean:write name="KureemuForm" property="y" /></a>
									<%} else {%>
										<a href="#" onClick="doSubmit('nextY')"><bean:write name="KureemuForm" property="yen" /></a>
									<%}%>
								</logic:notEqual>
							</TD>
				...
				<DIV class="mainlist">
					<TABLE>
						<THEAD>
							<TR>
								<TH colspan="2" width="11%"><%=i18n.get(GL.OS3101_KANJO_CD)%></TH>
								<TH width="49%"><%=i18n.get(GL.OS3101_KANJO_NM)%></TH>
								<TH colspan="3" width="16%"><%=i18n.get(GL.OS3101_COUNTRY)%></TH>
								<TH colspan="3" width="15%"><%=i18n.get(GL.OS3101_KINGAKU_TOTAL)%></TH>
								<TH width="9%"class="borderRight"><%=i18n.get(GL.OS3101_TAISYO_YM)%></TH>
							</TR>
							<TR>
								<TH width="6%"><%=SESSION_DATA_APP.getLbl_nm1()%></TH>
								<TH colspan="2" width="58%"><%=i18n.get(GL.OS3101_SOSHIKI)%></TH>
								<TH colspan="4" width="16%"><%=i18n.get(GL.OS3101_TANTO_NM)%></TH>
								<TH colspan="3" width="22%"class="borderRight"><%=i18n.get(GL.OS3101_PROGRESS)%></TH>
							</TR>
						</THEAD>
						<TBODY>
							<% if(KureemuForm.getList() != null) { %>
								<nested:iterate name="KureemuForm" property="list" indexId="idx">
									...
									<nested:notEqual property="hanki_sihanki_kbn" value="2">
										...
										<TD width="49%" ><nested:write property="kanjo_nm" />&nbsp;</TD>
									...
									<nested:notEqual property="hanki_sihanki_kbn" value="2">
										...
										<TD colspan="3" width="22%" class="borderBottom borderRight"><nested:write property="sintyoku" />&nbsp;</TD>
									</TR>
								</nested:iterate>
							<% } %>
						</TBODY>
